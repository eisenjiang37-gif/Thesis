...
综合来看，确定性信号+SVM准确率90.1\%已满足实用需求，训练时间仅0.011s，可在CPU上运行，端侧部署优势显著；CNN准确率最高（98.4\%），但训练耗时最长（172.1s），且需约1GB CUDA内存，适合云端部署；LSTM各项指标均最低，原因在于小样本场景（每类50--60样本）下易过拟合，且时序模型对相位噪声敏感、缺乏物理先验引导。
