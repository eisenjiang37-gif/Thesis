...
本研究定义了实现多点触控屏幕主要功能的手势，包括主/子界面切换、页面上下滚动和点击操作。基于图 2 中的 $x$-$y$-$z$ 坐标系，上述手势分别对应手掌在屏幕上方沿 $\pm x$、$\pm y$ 和 $\pm z$ 方向的运动，依次称为"左"（$L$）、"右"（$R$）、"前"（$F$）、"后"（$B$）和"点击"（$TAP$）手势。
...
实际操作中，水平手势易偏离 $x$ 轴和 $y$ 轴，引发误判。为此，额外定义了沿对角线方向（$y = x$ 和 $y = -x$）的冗余手势以最大化容错能力；一旦检测到对角运动，即将其判定为无效手势。这四类冗余手势（$LF$、$LB$、$RF$、$RB$）消除了 $L$、$R$、$F$、$B$ 之间的模糊区间。
...
此处采用从头计算（$ab \ initio$）方法，通过沿矩形孔径 $x$ 和 $y$ 方向积分 $y$ 极化赫兹偶极子的辐射，分析近场散射。在图 4 所示的全局 $x$-$y$-$z$ 坐标系中，设发射与接收天线相位中心分别位于 $(x_T, y_T, z_T)$ 和 $(x_R, y_R, z_R)$，垂直轴指向 $+z$ 方向，散射孔径几何中心位于 $(x_S, y_S, z_S)$，其法向亦沿 $+z$ 方向。
...
$I/Q$ 信号的差分功率 $\Delta P[n]$ 表征信号的瞬时变化，其中 $\Delta n_I[n] = n_I[n+1] - n_I[n]$，$\Delta n_Q[n]$ 同理，$\Delta P[n]$ 可计算为

\begin{equation}
\Delta P[n] = |\Delta \boldsymbol{\rho}[n]|^2.
\end{equation}

无运动时，$\Delta \boldsymbol{\rho}_{\text{noise}}[n] = (\Delta n_I[n], \Delta n_Q[n])$ 仅反映电路本底噪声。由于运动存在时 $\Delta \boldsymbol{\rho}$ 始终大于 $\Delta \boldsymbol{\rho}_{\text{noise}}$，因此 $\Delta P$ 可有效分离包含手势信息的信号段。
...
为验证分类器的跨用户泛化能力，另邀请四名受试者执行相同手势，进行留一受试者实验 $[12]$。每类手势重复 $10$ 次，受试者事先了解各手势的运动方向、手部姿势及有效感知区域。图 23 显示了由这些受试者检测到的极小值时刻组成的数据集的映射情况。可以看出，与图 21(a) 所示训练集的映射相比，这些极小值时刻也呈现出聚类且明显分离的状态。这表明，尽管不同受试者执行的手势之间存在差异，但经过重新缩放的极小值时刻对于 $OVR \ SVM$ 分类器而言仍表现出足够的相似性。
